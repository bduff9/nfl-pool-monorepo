...
import {
	...
	AvatarFallback,
	AvatarImage,
} from "@nfl-pool-monorepo/ui/components/avatar";
import { Button } from "@nfl-pool-monorepo/ui/components/button";
import {
	Collapsible,
	CollapsibleContent,
	CollapsibleTrigger,
} from "@nfl-pool-monorepo/ui/components/collapsible";
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
				...
				<SidebarMenu>
					<SidebarMenuItem
						className={cn(
							...
							user.doneRegistering !== 1 && "invisible",
						)}
					>
						<Button
							className={cn(
							...
							<DropdownMenuContent className="w-[--radix-popper-anchor-width]">
								{Array.from({ length: WEEKS_IN_SEASON }).map((_, i) => (
									<Fragment key={`week-${i + 1}`}>
										...
										<DropdownMenuItem onClick={() => setSelectedWeek(i + 1)}>
											Week {i + 1}
										</DropdownMenuItem>
										{i + 1 === currentWeek && <DropdownMenuSeparator />}
									</Fragment>
					...
					<SidebarMenuItem className="h-9 -m-2 text-center">
			...
			{user.doneRegistering === 1 ? (
				<SidebarContent>
					<Collapsible
						...
						defaultOpen={currentPage === "Dashboard"}
					>
						<SidebarGroup>
							...
							<CollapsibleContent>
								<SidebarGroupContent>
									<NavLink
										href="/picks/set"
										isNested
										show={myTiebreaker?.TiebreakerHasSubmitted !== 1}
									>
										Make Picks
									</NavLink>
					...
					<Collapsible
						className="group/collapsible"
						defaultOpen={currentPage === "Survivor"}
					>
						<SidebarGroup>
							<SidebarGroupLabel asChild className="text-2xl text-white">
								<CollapsibleTrigger>
								...
								<SidebarGroupContent>
									...
									<NavLink
										...
										show={
											!!user.playsSurvivor &&
											isAliveInSurvivor &&
											selectedWeekStatus === "Not Started"
										}
									>
										Make Picks
									</NavLink>
					...
					<SidebarMenuItem>
						<DropdownMenu>
							...
							<DropdownMenuContent align="end" side="right">
								{user.doneRegistering === 1 && (
									<DropdownMenuItem>
										<ProgressBarLink href="/users/edit">
											Edit Account
										</ProgressBarLink>
									</DropdownMenuItem>
								)}
								{user.doneRegistering === 1 && (
									<DropdownMenuItem>
										<ProgressBarLink href="/users/payments">
											View Payments
										</ProgressBarLink>
									</DropdownMenuItem>
								)}
								<DropdownMenuItem>
									<ProgressBarLink href="/auth/logout">
										Sign out
									</ProgressBarLink>
								</DropdownMenuItem>
							</DropdownMenuContent>
