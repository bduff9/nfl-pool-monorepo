...
const SurvivorTeamLoader: FC<SurvivorTeamLoaderProps> = ({ isHome = false }) => {
	return (
		<div
			className={cn('border-b border-e border-black relative pt-2 px-2 text-center w-1/2 h-[152px] flex flex-col items-center',
				!isHome && 'border-s',
			)}
		>
	...
	return (
		<div className="h-full flex">
			<div className="bg-gray-100/80 text-black mx-2 pt-5 md:pt-3 min-h-screen pb-4 flex-1">
				<div className="flex flex-wrap min-vh-100">
					<h4 className="w-full mb-5 text-center text-xl font-semibold">
						{SURVIVOR_PICK_INSTRUCTIONS}
					</h4>
					{Array.from({ length: 16 }).map((_, i) => (
						<div
							className={cn(
								'w-full md:w-1/2 lg:w-1/3 2xl:w-1/4 flex flex-wrap pb-3 relative',
							)}
			...
			>
				<div
					className={cn(
						'w-full text-muted border border-black flex justify-around overflow-hidden cursor-pointer h-[25px] items-center',
					)}
				>
					<div>
