...
import { Table, TableBody, TableCell, TableFooter, TableHead, TableHeader, TableRow } from '@nfl-pool-monorepo/ui/components/table';
				...
				<div className="bg-gray-100/80 text-black mx-2 pt-5 md:pt-3 min-h-screen pb-4 flex-1">
							...
							{payments.map(payment => {
								const amount = Number(payment.PaymentAmount);
									...
									return (
										<TableRow
											className={
												amount < 0 ? 'bg-red-300' : 'bg-green-300'
											}
					...
					{owed < 0 && (
						<PaymentSelector
							...
							defaultPayment={
								currentUser.UserPaymentType ?? 'Paypal'
							}
						/>
					)}
					...
					<TextSeparator>
						My Prize Payment Info
					</TextSeparator>
					<div className="mx-3">
						Payment Type: {currentUser.UserPaymentType}
					</div>
					<div className="mx-3">
						Payment Account: {currentUser.UserPaymentAccount}
					</div>
					<div className="mx-3">
