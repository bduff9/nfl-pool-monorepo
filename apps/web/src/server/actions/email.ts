...
import { getHtml as getCustomHtml, getPlainText as getCustomPlainText } from "@nfl-pool-monorepo/transactional/emails/templates/CustomEmail";
...
export const sendAdminEmail = authedProcedure
	...
	.handler(async ({ ctx, input }) => {
		...
		if (sendTo === 'All') {
			users = await db.selectFrom('Users').select(['UserEmail', 'UserFirstName']).where('UserCommunicationsOptedOut', '=', 0).execute();
		} else if (sendTo === 'New') {
			users = [{ UserEmail: userEmail, UserFirstName: userFirstName }];
		} else if (sendTo === 'Registered') {
			users = await db.selectFrom('Users').select(['UserEmail', 'UserFirstName']).where('UserCommunicationsOptedOut', '=', 0).where('UserDoneRegistering', '=', 1).execute();
		} else if (sendTo === 'Unregistered') {
			users = await db.selectFrom('Users').select(['UserEmail', 'UserFirstName']).where('UserCommunicationsOptedOut', '=', 0).where('UserDoneRegistering', '=', 0).execute();
		}

		for (const to of users) {
			try {
				if (emailType === 'Custom') {
					promises.push(sendCustomEmail({
						body,
						preview,
						subject,
						to,
		...
		.handler(async ({ input }) => {
			...
			try {
				await db.transaction().execute(async () => {
					...
					}).where('UserEmail', '=', email).executeTakeFirstOrThrow();
