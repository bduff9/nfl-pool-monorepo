...
export const loadAPICalls = cache(async (params: SearchParams) => {
	const paramsSchema = z.object({
		filter: stringToJSONSchema.pipe(z.array(z.object({ id: z.enum(['ApiCallWeek']), value: z.string() })).optional().default([])),
		page: z.coerce.number().optional().default(1),
		pageSize: z.union([z.coerce.number(), z.literal('all')]).optional().default(DEFAULT_PAGE_SIZE),
		sort: stringToJSONSchema.pipe(z.array(z.object({ desc: z.boolean(), id: z.enum(['ApiCallID', 'ApiCallUrl', 'ApiCallYear', 'ApiCallWeek', 'ApiCallDate']) })).optional().default([{ desc: true, id: 'ApiCallID' }])),
	});
