...
export const getAdminEmails = cache(async (params: SearchParams) => {
	const paramsSchema = z.object({
		filter: stringToJSONSchema.pipe(z.array(z.object({ id: z.enum(['EmailType']), value: z.string() })).optional().default([])),
		page: z.coerce.number().optional().default(1),
		pageSize: z.union([z.coerce.number(), z.literal('all')]).optional().default(DEFAULT_PAGE_SIZE),
		sort: stringToJSONSchema.pipe(z.array(z.object({ desc: z.boolean(), id: z.enum(['EmailType', 'UserName', 'EmailSubject', 'EmailCreatedAt']) })).optional().default([{ desc: true, id: 'EmailCreatedAt' }])),
	});

	const { filter, page, pageSize, sort } = paramsSchema.parse(params);

	let countResult = db.selectFrom('Emails as e').select(sql<number>`COUNT(*)`.as('count')).leftJoin('Users as u', 'u.UserEmail', 'e.EmailTo');
	let queryResult = db.selectFrom('Emails as e').select(['e.EmailID', 'e.EmailType', 'e.EmailTo', 'e.EmailSubject', 'e.EmailHtml', 'e.EmailSms', 'e.EmailCreatedAt']).leftJoin('Users as u', 'u.UserEmail', 'e.EmailTo').select(['u.UserName']);
...
export const getEmail = cache(async (emailID: SearchParams[string]) => {
	...
	const result = await db.selectFrom('Emails').select(['EmailHtml', 'EmailTo', 'EmailSubject']).where('EmailID', '=', emailID as string).executeTakeFirst();
