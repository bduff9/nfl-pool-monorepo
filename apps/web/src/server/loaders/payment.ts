...
export const getMyPayments = cache(async () => {
	...
	return db.selectFrom('Payments').select(['PaymentID', 'PaymentAmount', 'PaymentDescription', 'PaymentWeek']).where('UserID', '=', user.id).orderBy(eb => sql<string>`FIELD(${eb.ref('PaymentType')}, 'Fee', 'Paid', 'Prize', 'Payout') ASC`).execute();
});

export const getUserPayoutsForAdmin = cache(async (params: SearchParams) => {
	const paramsSchema = z.object({
		sort: stringToJSONSchema.pipe(z.array(z.object({ desc: z.boolean(), id: z.enum(['UserName', 'PaymentAmount', 'UserPaymentType']) })).optional().default([{ desc: false, id: 'UserName' }])),
	});
	const { sort } = paramsSchema.parse(params);

	let queryResult = db.selectFrom('Users as u')
		.select(({ selectFrom }) => [
			'u.UserID',
			'u.UserName',
			'u.UserTeamName',
		...
		.innerJoin('Payments as p', 'p.UserID', 'u.UserID')
		.where('p.PaymentType', '=', 'Prize');
