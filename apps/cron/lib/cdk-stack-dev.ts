...
export class CdkStackDev extends Stack {
	constructor(scope: Construct, id: string, props?: StackProps) {
		...
		const currentWeekUpdaterDev = new NodejsFunction(
			this,
			"CurrentWeekUpdaterDev",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		onceAnHourScheduleRule.addTarget(
			new targets.LambdaFunction(currentWeekUpdaterDev),
		);
		...
		const backupNflDatabaseDev = new NodejsFunction(
			this,
			"BackupNflDatabaseDev",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		twiceADayScheduleRule.addTarget(
			new targets.LambdaFunction(backupNflDatabaseDev),
		);

		const futureGameUpdaterDev = new NodejsFunction(
			this,
			"FutureGameUpdaterDev",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		twiceADayOnTheHalfHoursScheduleRule.addTarget(
			new targets.LambdaFunction(futureGameUpdaterDev),
		);
		...
		every5MinutesScheduleRule.addTarget(
			new targets.LambdaFunction(liveGameUpdaterDev),
		);
