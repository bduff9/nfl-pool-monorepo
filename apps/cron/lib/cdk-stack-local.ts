...
export class CdkStackLocal extends Stack {
	constructor(scope: Construct, id: string, props?: StackProps) {
		...
		const currentWeekUpdaterLocal = new NodejsFunction(
			this,
			"CurrentWeekUpdaterLocal",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		const onceAnHourScheduleRule = new events.Rule(
			this,
			"onceAnHourScheduleRule",
			{
				enabled: false,
		...
		onceAnHourScheduleRule.addTarget(
			new targets.LambdaFunction(currentWeekUpdaterLocal),
		);
		...
		const backupNflDatabaseLocal = new NodejsFunction(
			this,
			"BackupNflDatabaseLocal",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		const twiceADayScheduleRule = new events.Rule(
			this,
			"twiceADayScheduleRule",
			{
				enabled: false,
				schedule: events.Schedule.cron({
					day: "*",
		...
		twiceADayScheduleRule.addTarget(
			new targets.LambdaFunction(backupNflDatabaseLocal),
		);

		const futureGameUpdaterLocal = new NodejsFunction(
			this,
			"FutureGameUpdaterLocal",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		const twiceADayOnTheHalfHoursScheduleRule = new events.Rule(
			this,
			"twiceADayOnTheHalfHoursScheduleRule",
			{
				enabled: false,
				schedule: events.Schedule.cron({
					day: "*",
		...
		twiceADayOnTheHalfHoursScheduleRule.addTarget(
			new targets.LambdaFunction(futureGameUpdaterLocal),
		);

		const liveGameUpdaterLocal = new NodejsFunction(
			this,
			"LiveGameUpdaterLocal",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		const every5MinutesScheduleRule = new events.Rule(
			this,
			"every5MinutesScheduleRule",
			{
				enabled: false,
				schedule: events.Schedule.cron({
					day: "*",
		...
		every5MinutesScheduleRule.addTarget(
			new targets.LambdaFunction(liveGameUpdaterLocal),
		);
