...
export class CdkStackProd extends Stack {
	constructor(scope: Construct, id: string, props?: StackProps) {
		...
		const currentWeekUpdaterProd = new NodejsFunction(
			this,
			"CurrentWeekUpdaterProd",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		onceAnHourScheduleRule.addTarget(
			new targets.LambdaFunction(currentWeekUpdaterProd),
		);
		...
		const backupNflDatabaseProd = new NodejsFunction(
			this,
			"BackupNflDatabaseProd",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		twiceADayScheduleRule.addTarget(
			new targets.LambdaFunction(backupNflDatabaseProd),
		);

		const futureGameUpdaterProd = new NodejsFunction(
			this,
			"FutureGameUpdaterProd",
			{
				bundling: {
					externalModules: [],
					nodeModules: [],
		...
		twiceADayOnTheHalfHoursScheduleRule.addTarget(
			new targets.LambdaFunction(futureGameUpdaterProd),
		);

		const liveGameUpdaterProd = new NodejsFunction(
			this,
			"LiveGameUpdaterProd",
			{
				bundling: {
		...
		every5MinutesScheduleRule.addTarget(
			new targets.LambdaFunction(liveGameUpdaterProd),
		);
