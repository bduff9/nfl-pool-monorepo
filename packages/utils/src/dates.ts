...
export const getHoursBetweenDates = (date1: Date, date2 = new Date()): number =>
  Math.floor((date2.getTime() - date1.getTime()) / (MILLISECONDS_IN_SECOND * SECONDS_IN_MINUTE * MINUTES_IN_HOUR));
...
export const getOffsetString = (): string => {
  ...
  return `${offset > 0 ? "-" : "+"}${get2DigitNumber(offsetHours)}:${get2DigitNumber(offsetMins)}`;
};
