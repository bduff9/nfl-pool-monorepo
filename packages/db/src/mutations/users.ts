...
export const populateUserData = async (
	...
	user: Pick<
		Selectable<Users>,
		"UserID" | "UserEmail" | "UserPlaysSurvivor" | "UserTeamName" | "UserName"
	>,
): Promise<void> => {
	...
	if (week > 1) {
		...
		console.debug(
			`Inserted lowest score picks for user ${user.UserID}`,
			picksResult,
		);
		...
		console.debug(
			`Inserted lowest score tiebreakers for user ${user.UserID}`,
			tiebreakersResult,
		);
		...
		console.debug(
			`Inserted lowest score WeeklyMV for user ${user.UserID}`,
			weeklyMvResult,
		);
		...
		console.debug(
			`Inserted lowest score OverallMV for user ${user.UserID}`,
			overallMvResult,
		);
	} else {
		// Populate picks
		...
		console.debug(
			`Inserted tiebreakers for user ${user.UserID}`,
			tiebreakersResult,
		);
	}
...
export const registerUserForSurvivor = async (
	trx: Transaction<DB>,
	userId: number,
	updatedBy?: string,
) => {
	...
	for (const league of leagues) {
		...
		console.log(
			`Inserted survivor picks for user ${userId} and league ${league.LeagueID}`,
			result,
		);
	}
...
export const unregisterUserForSurvivor = async (
	...
	if (result.length === 0 && !override)
		throw new ZSAError("PRECONDITION_FAILED", "Season has already started!");
