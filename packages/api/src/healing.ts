...
import {
	findFutureGame,
	getGamesForWeek,
} from "@nfl-pool-monorepo/db/src/queries/game";
...
const movePointDown = async (
	pick: PoolPick,
	picks: Array<PoolPick>,
): Promise<void> => {
	...
	const highest = picks.reduce(
		(acc, pick) => {
			...
			if (
				acc === null ||
				acc.PickPoints === null ||
				acc.PickPoints < pick.PickPoints
			) {
				return pick;
			}
...
export const healPicks = async (week: number): Promise<void> => {
	...
	for (const { UserID, UserLeagues } of users) {
		for (const { LeagueID } of UserLeagues) {
			...
			console.log(`User's picks counted`, { ok, tooLow, tooHigh });
...
const redoGameNumbers = async (gameWeek: number): Promise<void> => {
	...
	for (let i = 0; i < games.length; i++) {
		...
		const foundGame = games.find(
			({ GameNumber }) => GameNumber === expectedGame,
		);
...
const findFutureAPIGame = (
	allAPIWeeks: NFLWeekArray,
	gameToFind: Game,
): [number, null | ApiMatchup] => {
	for (let i = 0; i < allAPIWeeks.length; i++) {
		...
		const found = apiWeek.matchup.find(({ team }) =>
			team.every(
				(team) =>
					(team.isHome === "1" &&
						team.id === gameToFind.homeTeam?.TeamShortName) ||
					(team.isHome === "0" &&
						team.id === gameToFind.visitorTeam?.TeamShortName),
			),
		);
	...
	for (let i = apiGames.length; i--; ) {
		...
		const gameIndex = currentDBWeek.findIndex(
			({ homeTeam, visitorTeam }) =>
				homeTeam?.TeamID === apiHomeTeamID &&
				visitorTeam?.TeamID === apiVisitorTeamID,
		);
		...
		if (gameIndex > -1) {
			...
			try {
				const futureGame = await findFutureGame(
					apiHomeTeamID,
					apiVisitorTeamID,
					week,
				);
