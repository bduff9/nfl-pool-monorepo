...
export const relativeTime = (date: Date): string => {
  ...
  if (Math.abs(diff) < MINUTES_IN_HOUR) {
    return relativeTimeFormatter.format(Math.round(diff), "minute");
  }

  diff /= MINUTES_IN_HOUR; // hours

  if (Math.abs(diff) < HOURS_IN_DAY) {
    return relativeTimeFormatter.format(Math.round(diff), "hour");
  }

  diff /= HOURS_IN_DAY; // days

  if (Math.abs(diff) < DAYS_IN_WEEK) {
    return relativeTimeFormatter.format(Math.round(diff), "day");
  }

  diff /= DAYS_IN_WEEK; // weeks

  if (Math.abs(diff) < WEEKS_IN_MONTH) {
    return relativeTimeFormatter.format(Math.round(diff), "week");
  }

  diff /= WEEKS_IN_MONTH; // years

  if (Math.abs(diff) < MONTHS_IN_YEAR) {
    return relativeTimeFormatter.format(Math.round(diff), "month");
  }

  diff /= MONTHS_IN_YEAR;

  return relativeTimeFormatter.format(Math.round(diff), "year");
};
