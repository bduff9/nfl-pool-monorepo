...
export const sendQuickPickConfirmationEmail = async (
  ...
  const game = await db
    ...
    .where("GameNumber", "=", 1)
    .executeTakeFirstOrThrow();
  const [selectedTeamId, notSelectedTeamId] =
    teamId === game.HomeTeamID ? [game.HomeTeamID, game.VisitorTeamID] : [game.VisitorTeamID, game.HomeTeamID];
  const selectedTeam = await db
    .selectFrom("Teams as t")
    .select(["t.TeamCity", "t.TeamName", "t.TeamPrimaryColor"])
    .where("t.TeamID", "=", selectedTeamId)
    .executeTakeFirstOrThrow();
  const notSelectedTeam = await db
    .selectFrom("Teams as t")
    .select(["t.TeamCity", "t.TeamName", "t.TeamPrimaryColor"])
    .where("t.TeamID", "=", notSelectedTeamId)
    .executeTakeFirstOrThrow();
